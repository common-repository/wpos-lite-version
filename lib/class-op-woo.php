...
class WPOSL_Woo{
    ...
    {
        $this->_session = new WPOSL_Session();
    ...
    public function order_columns_head($defaults){
        ...
        foreach($defaults as $key => $value)
        {
            ...
            if($key == 'cb')
            {
                $result['op_source']  = __('Source','wpos-lite');
    ...
    public function order_columns_content($column_name, $post_ID){
        if ($column_name == 'op_source') {
            ...
            if($source == 'wpos-lite')
            {
                echo '<img style="width: 16px;" alt="from openpos" src="'.WPOSL_URL.'/assets/images/shop.png">';
            }else{
                echo '<img style="width: 16px;" alt="from online website" src="'.WPOSL_URL.'/assets/images/woocommerce.png">';
            }
        }
    }
    ...
    public function order_pos_payment($order){
        ...
        if($source == 'wpos-lite')
        {
            ...
            <p class="form-field form-field-wide">
                <label><?php esc_html_e( 'POS Order Number:', 'wpos-lite'); ?> <b><?php echo esc_html($_pos_order_id)?></b></label>
            </p>
        <?php endif; ?>
            <?php if($_op_sale_by_person_id): $person = get_user_by('id',$_op_sale_by_person_id);  ?>
            <p class="form-field form-field-wide">
                <label><?php esc_html_e( 'Shop Agent:', 'wpos-lite'); ?> <b><?php echo esc_html($person->get('display_name') )?></b></label>
            </p>
            <?php endif; ?>
            <?php if(!empty($warehouse)): ?>
            <p class="form-field form-field-wide">
                <label><?php esc_html_e( 'Outlet:', 'wpos-lite'); ?> <b><?php echo esc_html($warehouse['name'] )?></b></label>
            </p>
            <?php endif; ?>
            <?php if(!empty($warehouse)): ?>
            <p class="form-field form-field-wide">
                <label><?php esc_html_e( 'Register:', 'wpos-lite'); ?> <b><?php echo isset($register['name']) ?  esc_html($register['name'] ) : __('Unknown','wpos-lite'); ?></b></label>
            </p>
            <?php endif; ?>
            <?php if($payment_methods): ?>
            <p class="form-field form-field-wide">
                <label><?php esc_html_e( 'POS Payment method:', 'wpos-lite'); ?></label>
                <ul>
            ...
            foreach($category_ids as $cat_id)
            {
                ...
                if($term && !empty($term))
                {
                    ...
                    $tmp  = array(
                        'id' => $cat_id,
                        'name' => $term['name'],
                        'image' => WPOSL_URL.'/assets/images/category_placehoder.png',
                        'description' => '',
                        'parent_id' => $parent_id,
                        'child' => array()
                    );
    ...
    public function get_shipping_method_by_code($code){
        ...
        $result = array(
                'code' => 'openpos',
                'title' => __('Custom Shipping','wpos-lite')
        );
    ...
    public function filter_orders_by_source(){
        ...
        if ( 'shop_order' === $typenow ) {
            $current = isset($_GET['_op_order_source']) ? sanitize_text_field($_GET['_op_order_source']) : '';
             ?>
                <select name="_op_order_source" id="dropdown_order_source">
                    <option value="">
                        <?php esc_html_e( 'Filter by Source', 'wpos-lite'); ?>
                    </option>
                    <option <?php echo ($current == 'online') ? 'selected':''; ?> value="online"><?php esc_html_e( 'Online Order', 'wpos-lite'); ?></option>
                    <option <?php echo ($current == 'pos') ? 'selected':''; ?> value="pos"><?php esc_html_e( ' POS Orders', 'wpos-lite'); ?></option>
    ...
    public function add_order_filterable_where($where, $wp_query){
        ...
        if ( 'shop_order' === $typenow && isset( $_GET['_op_order_source'] ) && ! empty( $_GET['_op_order_source'] ) ) {
            ...
            $source = isset($_GET['_op_order_source']) ? sanitize_text_field($_GET['_op_order_source']) : '';
            if($source == 'online')
            {
    ...
    public function woocommerce_after_order_itemmeta( $item_id, $item, $product){
        ...
        if($_op_local_id)
        {
            ...
            if($seller_id)
            {
                ...
                if($user)
                {
                    echo '<p>'.__('Seller: ','wpos-lite').'<strong>'.$user->display_name.'</strong></p>';
                    $has_seller = true;
                }
            ...
            if(!$has_seller)
            {
                echo '<p>'.__('Sold By Shop Agent','wpos-lite').'</p>';
            }
        }
    ...
    public function getCustomerAdditionFields(){
        ...
        $address_2_field = array(
            'code' => 'address_2',
            'type' => 'text',
            'label' =>  __('Address 2','wpos-lite'),
            'options' => array(),
            'placeholder' => __('Address 2','wpos-lite'),
            'description' => '',
            'onchange_load' => false,
            'allow_shipping' => 'yes',
        );
        ...
        $postcode_field = array(
            'code' => 'postcode',
            'type' => 'text',
            'label' =>  __('PostCode / Zip','wpos-lite'),
            'options' => array(),
            'placeholder' => __('PostCode / Zip','wpos-lite'),
            'description' => '',
            'onchange_load' => false,
            'allow_shipping' => 'yes',
        );
        ...
        $city_field = array(
            'code' => 'city',
            'type' => 'text',
            'label' =>  __('City','wpos-lite'),
            'options' => array(),
            'placeholder' => __('City','wpos-lite'),
            'description' => '',
            'onchange_load' => false,
            'allow_shipping' => 'yes',
        );
        ...
        $state_field = array(
            'code' => 'state',
            'type' => 'text',
            'label' =>  __('State','wpos-lite'),
            'options' => array(),
            'placeholder' => __('State','wpos-lite'),
            'description' => '',
            'onchange_load' => false,
            'allow_shipping' => 'yes',
        );
        ...
        $select_contry = array(
            'code' => 'country',
            'type' => 'select',
            'label' =>  __('Country','wpos-lite'),
            'options' => $country_options,
            'placeholder' => __('Choose Country','wpos-lite'),
            'description' => '',
    ...
    public function formatWooOrder($order_id){
        ...
        {
            $sale_person_name = __('Done via website','wpos-lite');
        }
    ...
    public function add_meta_boxes(){
        ...
        if($source == 'wpos-lite')
        {
            add_meta_box( 'look-openpos-order-setting',__('POS Information','wpos-lite'), array($this,'add_order_boxes'), 'shop_order', 'side', 'default' );
        }
    ...
    public function add_order_boxes(){
        ...
        <div class="openpos-order-meta-setting">
            ...
            <div style="width: 100%; float: left;">
                <a href="<?php echo admin_url('admin-ajax.php?action=print_receipt&id='.(int)$post->ID); ?>" target="_blank" style="background: transparent;padding: 0; float: right;border: none;"><image style="width: 28px;" src="<?php echo WPOSL_URL.'/assets/images/print.png'; ?>" /></a>
            </div>
            <?php endif; ?>
            <?php
    ...
    public function woocommerce_new_product($product_id){
        ...
        if($openpos_type == 'restaurant' && $product_id)
        {
            ...
            foreach($type_options as $akey => $aop) {
                ...
                {
                    $op_cook = sanitize_text_field($_REQUEST[$a_key]);
                    if($op_cook == 'on')
                    {
                        $cook = 'yes';
                    }
    ...
    public function woocommerce_order_item_display_meta_key($display_key, $meta){
        ...
        {
            $display_key = __('Item Details','wpos-lite');
        }
        return $display_key;
    }
