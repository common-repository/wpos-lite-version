...
{
    class WPOSL_Discounts extends  WC_Discounts {
        ...
        public function is_coupon_valid( $coupon,$request = array() ) {
            ...
            $request_params = array_map( 'sanitize_text_field', $_REQUEST);;
            $cart_data = isset($request_params['cart']) ? (array)json_decode(sanitize_textarea_field($request_params['cart']),true) : array();
            if($customer_email)
            {
                    ...
                    if(!empty($cart_data) && $op_session_data)
                    {
                        if(isset($cart_data['customer']) && $cart_data['customer']['email'])
                        {
                            if(!in_array(strtolower($cart_data['customer']['email']),$customer_emails))
                            {
                                return new WP_Error(
                                    'invalid_coupon',
                                    __('This coupon not for current customer','wpos-lite'),
                                    array(
                                        'status' => 400,
                                    )
                                );
                            ...
                            return new WP_Error(
                                'invalid_coupon',
                                __('Please add customer before valid coupon','wpos-lite'),
                                array(
                                    'status' => 400,
                                )
                            );
