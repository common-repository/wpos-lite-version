...
class Openpos_Core
{
    ...
    public function __construct()
    {
        ...
        // check license
        $this->plugin_info = get_plugin_data(WPOSL_DIR.'index.php');
        //check requirement ....
    ...
    function additionPaymentMethodDetails(){
        ...
        $payment_options['chip_pin'] = array(
           'code' => 'chip_pin',
           'admin_title' => __('Chip & PIN','wpos-lite'),
           'frontend_title' => __('Chip & PIN','wpos-lite'),
            'description' => ''
        );

        $payment_options['stripe'] = array(
            'code' => 'stripe',
            'admin_title' => __('Credit Card ( Stripe ) - Online Payment for POS only','wpos-lite'),
            'frontend_title' => __('Credit Card','wpos-lite'),
            'description' => ''
        );
    ...
    function formatPaymentMethods($methods){
        ...
        foreach($methods as $code => $title)
        {
            ...
            if($code == 'chip_pin')
            {
                $description = __('Click Generate to get a reference order number. Then process the payment using your chip & PIN device.','wpos-lite');
            }
    ...
    public function formatReceiptSetting($setting,$op_incl_tax_mode= false)
    {
        ...
        $theme_file_path = rtrim(get_stylesheet_directory(),'/').'/wpos-lite/'.$file_name;
        ...
        $gift_file_path = rtrim(WPOSL_DIR,'/').'/default/'.$_gift_file_name;

        $gift_theme_file_path = rtrim(get_stylesheet_directory(),'/').'/wpos-lite/'.$_gift_file_name;
        ...
        if($this->_filesystem->is_file($file_path))
        {
            ...
            <tr class="tabletitle">
                <td class="item"><h2>'.__('Item','wpos-lite').'</h2></td>
                <td class="qty"><h2>'.__('Price','wpos-lite').'</h2></td>
                <td class="qty"><h2>'.__('Qty','wpos-lite').'</h2></td>
                <td class="qty"><h2>'.__('Discount','wpos-lite').'</h2></td>
                <td class="total"><h2>'.__('Total','wpos-lite').'</h2></td>
            </tr>
            ...
            <tr class="tabletitle">
                <td></td>
                <td class="Rate sub-total-title" colspan="3"><h2>'.__('Sub Total','wpos-lite').'</h2></td>
                <td class="payment sub-total-amount"><h2><%= sub_total %></h2></td>
            </tr>
            <% if(shipping_cost > 0) {%>
            <tr class="tabletitle">
                <td></td>
                <td class="Rate shipping-title" colspan="3"><h2>'.__('Shipping','wpos-lite').'</h2></td>
                <td class="payment shipping-amount"><h2><%= shipping_cost %></h2></td>
            </tr>
            <% } %>
            <tr class="tabletitle">
                <td></td>
                <td class="Rate cart-discount-title" colspan="3"><h2>'.__('Discount','wpos-lite').'</h2></td>
                <td class="payment cart-discount-amount"><h2><%= final_discount_amount %></h2></td>
            </tr>
            <tr class="tabletitle">
                <td></td>
                <td class="Rate tax-title" colspan="3"><h2>'.__('Tax','wpos-lite').'</h2></td>
                <td class="payment tax-amount"><h2><%= tax_amount %></h2></td>
            </tr>
            <tr class="tabletitle">
                <td></td>
                <td class="Rate grand-total-title" colspan="3"><h2>'.__('Grand Total','wpos-lite').'</h2></td>
        ...
        if(isset($setting['openpos_type']) && $setting['openpos_type'] == 'restaurant')
        {
            ...
            $kitchen_file_path = rtrim(WPOSL_DIR,'/').'/default/'.$file_name;
    ...
    public function getReceiptFontCss(){
        $font_css = array();
        $font_css['receipt_font'] = WPOSL_URL.'/pos/font.css';
        return $font_css;
    }
